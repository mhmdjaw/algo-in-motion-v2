export interface PahtfindingAnimation {
...
export const pathfinding = (maze: number[][]): PahtfindingAnimation[] => {
  const animations: PahtfindingAnimation[] = []
  ...
  while (openList.length !== 0) {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      for (let j = 0; j < closedList.length; j++) {
        if (child.row === closedList[j].row && child.col === closedList[j].col) {
          closed = true
          break
        }
      }
      ...
      child.h = Math.abs(endCell.row - child.row) + Math.abs(endCell.col - child.col)
